...
class TrainUpit(object):
    ...
    def prep_data(self, mix, sources):
        pass
    ...
    def train(self, train_data):
        ...
        for batch_i in range(0, n, self.batch_size):
            ...
            batch_set = sorted([train_data[i] for i in batch_indices],
                    ...
                    reverse=True)
            dprint(f"Batch indices: {batch_indices}")
            if DEBUG_LEVEL >= 2:
                for i, (mix, sources) in enumerate(batch_set):
                    dprint(f"  {i:2d} Mix: {mix.shape}, sources: {[s.shape for s in sources]}", level=2)
            ...
            if DEBUG_LEVEL >= 2:
                for i, src in enumerate(sources_spec):
                    dprint(f" source_spec {i:3d}: {src.size()}")
                for i, src in enumerate(sources_phase):
                    dprint(f" source_phase {i:3d}: {src.size()}")
            dprint(f"sources_spec {len(sources_spec)}", level=2)
            dprint(f"sources_phase {len(sources_phase)}", level=2)
            ...
            model_input = model_input.cuda() if th.cuda.is_available() else model_input.to(DEVICE)

            self.optimizer.zero_grad()

            print(f"  input_sizes: {input_sizes.size()}")
            print(f"  model_input: {type(model_input)}")
            # compute the output masks
            est_masks = self.model(model_input)

            print(f"  est_masks: {type(est_masks)} {len(est_masks)}")
    ...
    def total_permutation_loss(self, est_masks, input_sizes,
        ...
        def per_permutation_loss(permutation):
            ...
            dprint(f"per_permutation_loss()")
            loss = []
            for mix_i, src_i in enumerate(permutation):
                dprint(f"  permute: {mix_i}, {src_i}")
                dprint(f"  mix_phase: {mix_phase.size()}")
                dprint(f"  sources_spec: {sources_spec[src_i].size()}")
                ...
                dprint(f"  spect: {spect.size()}")
                dprint(f"  mix_spec: {mix_spec.size()}")
                dprint(f"  est_masks: {est_masks[mix_i].size()}")
                full_utt_loss = th.sum(
                        th.sum(
                            th.pow(est_masks[mix_i] * mix_spec - spect, 2), -1),
                        -1)
